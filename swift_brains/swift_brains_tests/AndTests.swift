...
class AndTests {
    public func test() {
        ...
        print("Expecting 1.0 got \(nn.predict(input: [1.0, 1.0]))")
        print("Expecting 0.0 got \(nn.predict(input: [0.0, 1.0]))")
        print("Expecting 0.0 got \(nn.predict(input: [1.0, 0.0]))")
    ...
    private func generateData() -> ([[Double]], [[Double]]) {
        ...
        for i in 0...500 {
            ...
            output.append([i % 3 == 0 && i % 5 == 0 ? 1.0 : 0.0])
        }
        return (input, output)
    }
}
