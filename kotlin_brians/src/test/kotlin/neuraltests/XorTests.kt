...
class XorTests {
    private val topology: Array<Int> = arrayOf(2, 2, 2, 1)

    @Test
    fun given_1_1_shouldBe_0() {
        // Arrange
        val sut = NeuralNet(topology)
        val (ins, out) = getData()

        // Act
        sut.train(ins, out, 1000)
    ...
    private fun getData(): Pair<List<Array<Double>>, List<Array<Double>>> {
        ...
        for(i in 0..500) {
            ...
            val isXlusive = (i % 3 == 0 && i % 5 != 0) || (i % 3 != 0 && i % 5 == 0)
            val out = arrayOf(if(isXlusive) 1.0 else 0.0)
            pair.first.add(ins)
            pair.second.add(out)
        }
        return pair
